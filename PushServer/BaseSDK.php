...
use BaiduBundle\PushBundle\PushServer\BAIDU_PUSH_CONFIG;
use BaiduBundle\PushBundle\PushServer\HttpRequest;
use BaiduBundle\PushBundle\PushServer\PushSimpleLog;
use BaiduBundle\PushBundle\PushServer\AssertHelper;

use BaiduBundle\PushBundle\PushServer\ServerException;
use BaiduBundle\PushBundle\PushServer\HttpException;
use BaiduBundle\PushBundle\PushServer\ClientException;
...
class BaseSDK{
    ...
    protected function onError($requestId = null, $code = 0, $msg = ''){
        ...
        if ($code > 0 && $code < 100) {
            ...
            $type = self::ERR_NET;
			$err = new HttpException("[$code] $msg", $code);
        } else if ($code >= 10000) {
            // 大于10000应为服务端返回的错误
            $type = self::ERR_SERVER;
			$err = new ServerException("[$code] $msg", $code);
        } else {
            $code = - 1;
